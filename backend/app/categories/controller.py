...
from ..models.base import CategoryEnum, RatingEnum
from ..models.ratings import Rating
from ..core.controller import get_users_route
from ..models.user import UserRating
...
async def rate_item_route(category_id: CategoryEnum, item_id: ObjectID, rating: float,
    ...
    user = None
    async for item in db.core.users.find():
        if item["username"] == user_name:
            user = item
            break

    # return success false if user is invalid
    if user == None:
        return {"success": False}
    ...
    rating_here = Rating(user_id=user_id, rating=rating, item_id=item_id)
    ...
    if existing == None:
        ...
        _res = await db[category_id]["ratings"].insert_one(rating_here.dict())
        # add the new rating
        rating_list.append(UserRating(
            rating_id=_res.inserted_id, rating=rating).dict())
        # update new rating in core.users.rating
        db.core.users.update_one(token, {"$set": {"ratings": rating_list}})
    else:
        # update the previous rating in {category}.ratings
        await db[category_id]["ratings"].update_one({"user_id": user_id, "item_id": item_id}, {"$set": {"rating": rating}})
        # remove the previous rating
        rating_list.remove(UserRating(
            rating_id=existing["_id"], rating=existing["rating"]).dict())
        # add the new rating
        rating_list.append(UserRating(
