                ...
                <v-col cols="12" xs="12" class="align-center">
                    <v-btn-toggle rounded>
                        <v-btn
                            outlined
                            large
                            @click="$emit('previous', !$v.form.$invalid)"
                            :color="$v.form.$invalid ? 'error' : 'success'"
                        >
                        ...
                        <v-btn
                            outlined
                            large
                            :color="$v.form.$invalid ? 'warning' : 'error'"
                            @click="$v.form.$invalid ? 'reset' : 'cancel'"
                        >
                            <v-icon left>{{
                                $v.form.$invalid ? 'mdi-refresh' : 'mdi-close'
                            }}</v-icon>
...
export default {
    ...
    data() {
        const data = {
            ...
            listeningChoices: ['youtube', 'spotify', 'gaana', 'prime', 'apple', 'saavn'],
    ...
    computed: {
        ...
        languageErrors() {
            ...
            errors = [...getRequiredErrors(!this.$v.form.language.required, 'language')];
            return errors;
        },
        directorErrors() {
            let errors = [];
            if (!this.$v.form.director.$dirty) return errors;
            errors = [...getRequiredErrors(!this.$v.form.director.required, 'director')];
            return errors;
        },
        streamingErrors() {
            let errors = [];
            if (!this.$v.form.streaming.$dirty) return errors;
            errors = [...getRequiredErrors(!this.$v.form.streaming.required, 'streaming')];
            return errors;
        },
        episode_lengthErrors() {
            ...
            errors = [
                ...getRequiredErrors(!this.$v.form.episode_length.required, 'Episode Length'),
        ...
        season_lengthErrors() {
            ...
            errors = [
                ...getRequiredErrors(!this.$v.form.season_length.required, 'Season Length'),
        ...
        artistErrors() {
            ...
            errors = [...getRequiredErrors(!this.$v.form.artist.required, 'Artist')];
            return errors;
        },
        authorErrors() {
            let errors = [];
            if (!this.$v.form.author.$dirty) return errors;
            errors = [...getRequiredErrors(!this.$v.form.author.required, 'Author')];
            return errors;
        }
    }
};
